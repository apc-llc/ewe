...
{
  if (_youngs_moduli.size()   != 3) mooseError("CardiacWhiteley2007Material: youngs_moduli must contain exactly 3 entries.");
  if (_poissons_ratios.size() != 3) mooseError("CardiacWhiteley2007Material: poissons_ratios must contain exactly 3 entries.");
  if (_shear_moduli.size()    != 3) mooseError("CardiacWhiteley2007Material: shear_moduli must contain exactly 3 entries.");
...
CardiacWhiteley2007Material::~CardiacWhiteley2007Material()
{
  delete _local_elasticity_tensor;
}

void
CardiacWhiteley2007Material::computeProperties()
{
